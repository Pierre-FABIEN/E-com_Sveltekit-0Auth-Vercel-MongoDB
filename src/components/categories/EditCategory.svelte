<script lang="ts">
	...
	// Stores for category data
	const categoryData = writable({
		name: '',
		price: 0,
		images: [] as File[],
		description: '',
	...
	// Function to initialize the stores with category data
	const initializeProductData = () => {
		categoryData.set({
			...
			categoryId: category.categories.map((category: any) => category.categoryId)
		});
	};

	// Initialize category data when the component is mounted
	onMount(() => {
	...
	$: updateProductData = categoryData;
...
<Sheet.Content side="right">
	...
	>
		...
		<div class="ccs mt-5">
			<div class="w-[100%]">
				<Form.Field name="name" form={updateProduct}>
					<Form.Control let:attrs>
						...
						<Input {...attrs} type="text" bind:value={$categoryData.name} />
					</Form.Control>
					<Form.FieldErrors />
				</Form.Field>
			</div>
			...
			<div class="w-[100%]">
				<Form.Field name="price" form={updateProduct}>
					<Form.Control let:attrs>
						...
						<Input {...attrs} type="number" bind:value={$categoryData.price} />
					</Form.Control>
					<Form.FieldErrors />
				</Form.Field>
			</div>
			...
			<div class="w-[100%]">
				<Form.Field name="description" form={updateProduct}>
					<Form.Control let:attrs>
						...
						<Input {...attrs} type="text" bind:value={$categoryData.description} />
					</Form.Control>
					<Form.FieldErrors />
				</Form.Field>
			</div>
			...
			<input type="hidden" name="categoryId" value={$categoryData.categoryId.join(',')} />
		</div>
		<Sheet.Footer>
